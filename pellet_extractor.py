...
def extract_pellet(img_bgr, tube_mask, bbox,
    ...
    """
    Define the liquid-pool ROI for one tube by geometry (bottom cone).

    Returns a dict (always, never None -- measurement is unconditional):
        pellet_mask  full-image boolean ROI mask (name kept for compatibility)
        valid_mask   same as pellet_mask
        roi_mask     alias, clearer name
        pixel_count  number of ROI pixels
        valid_pixel_count  same
        reliable     bool, False if ROI is suspiciously small
        roi_bottom_frac    the fraction used (diagnostic)
    """
    x, y, w, h = bbox
