...
def load_sam(model_type="vit_b", checkpoint="sam_vit_b_01ec64.pth", device=None):
    ...
    mask_generator = SamAutomaticMaskGenerator(
        model=sam,
        points_per_side=16,             # was 32 -- for ONE large tube on a plain
                                        #   background, 16 (256 pts) finds it fine
                                        #   and is ~4x cheaper than 32. Raise back
                                        #   toward 32 only if detection misses.
...
def detect_tubes(img, mask_generator,
    ...
    seg_max_dim    : int    downscale the image so its longest edge is at most
                            this many pixels BEFORE running SAM, then upscale the
                            returned masks back to full resolution. Segmentation
                            cost scales with pixel count, so this is the main
                            speed lever. Color is still measured on the full-res
                            image downstream, so accuracy is unaffected. Set to
                            None/0 to disable. ~1024 finds one large tube easily.
    debug          : bool   if True, print per-mask geometry for tuning.

    Returns
    -------
    list of dicts: bbox (x,y,w,h), mask (bool), area (int), score (float)
                   -- all in FULL-resolution image coordinates.
    """
    H, W = img.shape[:2]
    img_area = H * W

    # Downscale just for segmentation; remember the factor to upscale masks back.
    if seg_max_dim and max(H, W) > seg_max_dim:
        scale = seg_max_dim / float(max(H, W))
        small = cv2.resize(img, (int(round(W * scale)), int(round(H * scale))),
                           interpolation=cv2.INTER_AREA)
        if debug:
            print(f"[detect_tubes] segmenting at {small.shape[1]}x{small.shape[0]} "
                  f"(downscaled from {W}x{H}), measuring at full res")
    else:
        small = img
    ...
    # Upscale every mask back to full resolution so all geometry + color work
    # downstream happens in original-image coordinates.
    if small.shape[:2] != (H, W):
        for mrec in masks:
            seg = mrec["segmentation"].astype(np.uint8)
            mrec["segmentation"] = cv2.resize(
                seg, (W, H), interpolation=cv2.INTER_NEAREST).astype(bool)
