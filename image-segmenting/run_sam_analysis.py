...
parser.add_argument("--readout", choices=["fluid", "wall"], default="fluid",
                    help="Which ROI to measure. 'fluid' = bottom supernatant "
                         "pool (colored = positive). 'wall' = tube body wall "
                         "adhesion stain (colored = NEGATIVE, clear = positive).")
...
# Estimate per-image illumination from a tube-free background patch. Build a
# mask of all detected tubes, dilate it, then sample background pixels far from
# any tube. The mean background RGB is the per-image illuminant estimate used
# downstream for lighting normalization.
_tube_union = np.zeros(img.shape[:2], dtype=bool)
for t in tubes:
    _tube_union |= t["mask"]
_dil = cv2.dilate(_tube_union.astype(np.uint8),
                  cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (41, 41)), 1).astype(bool)
_bg = ~_dil
if _bg.sum() > 100:
    _bg_bgr = img[_bg].mean(axis=0)
    bg_R, bg_G, bg_B = float(_bg_bgr[2]), float(_bg_bgr[1]), float(_bg_bgr[0])
else:
    bg_R = bg_G = bg_B = float("nan")
print(f"Background illuminant RGB: ({bg_R:.1f}, {bg_G:.1f}, {bg_B:.1f})")
